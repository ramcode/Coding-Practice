...
public class BinaryTree<T extends Comparable<T>> {
		...
		if (node != null) {
			...
			while (!queue.isEmpty()) {
				...
				while(level>0){
					...
					if(current.getParentNode()!=null && current.getData().compareTo(current.getParentNode().getData())>0){
						sb.append("\t");
					}
	...
	public Node<T> findMinimum(Node<T> root){
		if (root.getLeftNode()==null) return root;
		else{
	...
	public static void main(String arg[]) {
		...
		System.out.println(bt.printInorderTraversal(bt.getRootNode()));
		//System.out.println(bt.findSuccesor(bt.getRootNode(), new Node<Integer>(12)).getData());
